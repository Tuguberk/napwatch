...
Builds from source (Homebrew pulls in `rust` as a build dependency), so the first install takes a minute or two.

> Runs `brew update` under the hood if it's been a while — on an old, cluttered Homebrew setup that can trigger `brew autoremove` cleanup as a side effect. Run `brew autoremove --dry-run` first if you want to preview that.
